...
async function getLocalidad(id){
    const localidad = localidades.find(localidad => localidad.id === Number(id))
    return localidad || null
}

async function getLocalidadPorProvincia(id){
    const localidadesFiltro = localidades.filter(localidad => localidad.id_provincia === Number(id))
